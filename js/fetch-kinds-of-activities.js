...
const selectorInputTel = document.querySelectorAll('input[type="tel"]');
const productArray = [];
...
function printQuantity() {
    const length = cartProductList.children.length;
    cartQuantity.textContent = length;
    cartEl.classList.toggle("active", length > 0);
    shoppingCart.classList.toggle("active", length > 0);
  }
...
function renderActivitiesList(activities) {
   ...
   heartBtn.forEach(btn => {
    btn.addEventListener("click", (e) => {
      ...
      cartProductList.insertAdjacentHTML("afterbegin", generateCartProduct(imgEl, titleEl, parent.id, descriptionEl, dateEl, timeEl ));
      printQuantity();
  ...
  cartProductList.addEventListener("click", (e) => {
    if (e.target.classList.contains("fa-trash")) {
      const productParent = e.target.closest(".cart-content__item");
...
function generateCartProduct(image, title, id) {
    return `
